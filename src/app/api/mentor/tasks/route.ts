...
// GET  /api/mentor/tasks?track=PYTHON  → list tasks for a track
// POST /api/mentor/tasks               → create a task
// PATCH /api/mentor/tasks              → update a task (only own)
// DELETE /api/mentor/tasks?taskId=xxx  → delete a task (only own)

export async function GET(request: NextRequest) {
  const auth = await requireAuth(request, ["mentor", "admin"]);
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    // Mentors can only edit their own tasks (admins can edit any)
    if (auth.session.role === "mentor" && data.mentor_id !== auth.session.id) {
