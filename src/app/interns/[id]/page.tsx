...
import { Loader2, Mail, MapPin, GraduationCap, Calendar, Globe } from 'lucide-react';
  ...
  return (
    <div className="max-w-4xl mx-auto pb-12">
      <div className="mb-6 flex items-center gap-2 text-sm">
        <Link href="/interns" className="text-gray-500 hover:text-white transition-colors">Intern Directory</Link>
      ...
      <div className="rounded-2xl border overflow-hidden" style={{ background: 'rgba(17,24,39,0.5)', borderColor: 'rgba(255,255,255,0.08)' }}>
        {/* Cover / Header Area */}
        <div className="h-32 md:h-48 w-full" style={{ background: 'linear-gradient(135deg, rgba(34,211,238,0.2), rgba(59,130,246,0.2))' }} />
        ...
        <div className="px-6 md:px-10 pb-8 relative">
          <div className="flex flex-col md:flex-row md:items-end gap-6 -mt-16 md:-mt-20 mb-6">
            ...
            <div className="flex-1 min-w-0 pb-2">
              ...
              <p className="text-gray-400 text-base">{profile?.department || 'Intern'}</p>
            </div>
                ...
                <div className="pt-6 border-t border-white/5">
                  ...
                  <div className="flex flex-col gap-3">
                    {user.social_links.linkedin && (
                      <a href={user.social_links.linkedin} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-sm text-gray-300 hover:text-cyan-400 transition-colors">
                        <Globe className="w-4 h-4 text-gray-500" /> LinkedIn
                      </a>
                    )}
                    {user.social_links.github && (
                      <a href={user.social_links.github} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-sm text-gray-300 hover:text-cyan-400 transition-colors">
                        <Globe className="w-4 h-4 text-gray-500" /> GitHub
                      </a>
                    )}
                    {user.social_links.portfolio && (
                      <a href={user.social_links.portfolio} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-sm text-gray-300 hover:text-cyan-400 transition-colors">
                        <Globe className="w-4 h-4 text-gray-500" /> Portfolio
                      </a>
                    )}
                    {user.social_links.twitter && (
                      <a href={user.social_links.twitter} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-sm text-gray-300 hover:text-cyan-400 transition-colors">
