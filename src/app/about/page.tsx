        ...
        <div className="relative z-10 max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          ...
          <AnimateOnScroll variant="fadeUp">
            <div className="space-y-8">
              ...
              {/* Founder card */}
              <div className="flex items-center gap-4 p-5 rounded-2xl bg-white/80 dark:bg-neutral-900 border border-slate-100 dark:border-neutral-800 shadow-sm">
                ...
                <div>
                  <div className="font-bold text-slate-900 dark:text-white text-sm">Suleman Zaheer</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
            {team.map((member, i) => (
              <AnimateOnScroll key={member.name} variant="fadeUp" delay={0.1 * i} className="h-full">
                <div className="group/card relative rounded-3xl overflow-hidden border border-slate-200 dark:border-neutral-800 bg-white/80 dark:bg-neutral-900 aspect-[3/4] hover:border-brand-300 dark:hover:border-brand-700 hover:shadow-2xl transition-all duration-500 h-full flex flex-col">
                  <Image
                    src={member.image}
                    alt={member.name}
                    fill
                  ...
                  <div className="absolute inset-0 p-6 flex flex-col justify-end">
                    ...
                    {(member.linkedin || member.github) && (
                      <div className="pt-4 mt-4 border-t border-white/20 flex gap-2 lg:opacity-0 lg:-translate-y-2 group-hover/card:opacity-100 group-hover/card:translate-y-0 transition-all duration-300">
                        {member.linkedin && (
                          <a href={member.linkedin} target="_blank" rel="noopener noreferrer" className="p-2 rounded-lg bg-white/10 hover:bg-brand-500 border border-white/20 hover:border-brand-500 text-white transition-colors">
                            <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                          </a>
                        )}
                        {member.github && (
                          <a href={member.github} target="_blank" rel="noopener noreferrer" className="p-2 rounded-lg bg-white/10 hover:bg-slate-700 border border-white/20 hover:border-slate-700 text-white transition-colors">
